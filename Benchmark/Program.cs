...
namespace JsonBenchmark
{
	class Program
	{
		enum BenchTarget
		{
			RevenjManual, RevenjJsonNet, Jil, JsonNet, RevenjProtoBuf
		}
		...
		{
			//args = new[] { "JsonNet", "Large", "Both", "100" };
			if (args.Length != 4)
			{
				Console.WriteLine(
		...
		static void SetupRevenj(out Action<object, Stream> serialize, out Func<Stream, Type, object> deserialize, bool? manualJson)
		{
			var json = manualJson == true
			...
			serialize = (obj, stream) => serialization.Serialize(obj, json, stream);
			deserialize = (stream, type) => serialization.Deserialize(stream, type, json, default(StreamingContext));
		}
